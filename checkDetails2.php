        ...
        while ($row = $weekday->fetch_assoc()){
            for($i=0; $i<$days; $i++){
                if(weekday($date[$i]) == $row["detDay"]){
                    echo '<form method="post" action="bookDetails.php"><tr></td><td class = "td1">'."Date and Time: </td><td class = \"td1\">".$date[$i]." ".$row["depTime"].'</td></td><td class = "td1">'."Flight Time: ".$row["flightTime"].'</td><td  class = "td1" ><input type="submit" value="BOOK"></td></tr>';
                    echo "<input type='hidden' name='book_date' value='$date[$i]'>";
